/*!
 * tx.js - transaction object for decentraland
...
TX.prototype.isSane = function isSane(ret) {
  ...
  if (this.outputs.length !== this.inputs.length) {
    ret.reason = 'bad-txns-different-number-inputs-outputs';
    ret.score = 100;
    return false;
  }
...
TX.prototype.checkInputs = function checkInputs(view, height, ret) {
  ...
  for (i = 0; i < this.inputs.length; i++) {
    ...
    if (coin.x !== this.outputs[i].x ||
        coin.y !== this.outputs[i].y) {
      ret.reason = 'bad-txns-incorrect-xy';
      ret.score = 100;
      return false;
    }
...
TX.prototype.getFee = function getFee(size, rate) {
  return 0;
};
