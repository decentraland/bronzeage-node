...
Chain.prototype._init = function _init() {
  ...
  this.webtorrent.on('error', function (err) {
...
Chain.prototype.setTile = co(function* setTile(x, y, contentFile) {
  ...
  var dirPath = this.options.prefix + 'tiles' + path.sep + x + path.sep + y + path.sep;
  const copyPath = dirPath + x + '.' + y + '.tld';
  mkdirp.sync(dirPath);
  var copy = fs.createReadStream(contentFile)
  ...
  copy.on('finish', () => {
    ...
    self.webtorrent.seed(fs.createReadStream(copyPath), (torrent) => {
      self.logger.info('WebTorrent is seeding:', torrent.infoHash);
      self.logger.info('file', torrent.torrentFile);
      console.log('Total torrents:', self.webtorrent.torrents.length);
    });
  });
...
Chain.prototype.getBoundaryTile = co(function* getBoundaryTile() {
  // TODO: must optimize, doing BFS for now
  const queue = [];
  let begin = 0;
  const seen = {};
  ...
  while (begin < queue.length) {
    ...
    for (let i=0; i<4; i++) {
      ...
      markSeen(x,y);
      const exists = yield this.db.getTile(x,y);
      if (!exists) {
        available.push({x,y});
      } else {
        queue.push({x,y});
