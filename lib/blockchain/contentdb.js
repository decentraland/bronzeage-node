...
ContentDB.prototype._seedFile = co(function* _seedFile(x, y) {
  var self = this;
...
ContentDB.prototype.copy = co(function* copy(from, to) {
...
ContentDB.prototype.save = co(function* save(x, y, hash, stream, isCurrent) {
  ...
  yield stream.pipe(fs.createWriteStream(historicPath));
  if (isCurrent) {
    this.logger.info(`Saving new version of ${x}, ${y} at ${historicPath}`);
    return this.copy(historicPath, currentPath);
  }
});
...
ContentDB.prototype.fetch = co(function* fetch(x, y, hash) {
  ...
  if (hash === constants.NULL_HASH) {
    const hola = fs.createReadStream('~/hola.txt');
    return this.save(x, y, hash, hola, true);
  }
