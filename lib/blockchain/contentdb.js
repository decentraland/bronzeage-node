...
const NAME_REGEX = new RegExp('^(-?\\d+)\\.(-?\\d+)\\.tld$');
...
ContentDB.prototype._getCoorsFromFileName = function (file) {
  ...
  return { x: parseInt(res[0], 10), y: parseInt(res[1], 10) };
};
...
ContentDB.prototype.save = co(function* save(x, y, hash, stream, isCurrent) {
  ...
  save.on('finish', () => {
    if (isCurrent) {
      ...
      var torrent = self.webtorrent.get(hash);
      if (!torrent) {
        this._seed(x , y);
      }
    }
...
ContentDB.prototype.putFile = co(function* putFile(x, y, contentFile) {
  var self = this;
