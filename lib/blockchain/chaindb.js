...
ChainDB.prototype._open = co(function* open() {
  ...
  var self = this;
  this.content.on('content', function *(x, y, file, hash) {
    console.log('CONTENTTTTTTTTTTTT', file);
    var tile = yield self.getTile(x,y);
    console.log('downloaded content for ', hash, 'and current tile is', tile.content);
    // if downloaded content for x,y is current content, save file
    var current = tile.content === hash;
    self.content.save(x, y, hash, file.createReadStream(), current);  
  });
