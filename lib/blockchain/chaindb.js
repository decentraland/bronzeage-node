...
ChainDB.prototype.indexTX = co(function* indexTX(tx, view, entry, index) {
  ...
  console.log('indeeeeeeeeeex tx', hash.toString('hex'), tx.outputs[0].x, tx.outputs[0].y);
  for (i = 0; i<tx.outputs.length; i++) {
    output = tx.outputs[i];
    // save previous tile for undo
    console.log('save previous tile for undo');
    const xyKey = layout.M(output.x, output.y);
    var prevHash = yield this.db.get(xyKey);
    console.log('got previous tile for undo', prevHash);
    if (prevHash) {
      this.put(layout.MU(output.x, output.y, hash), prevHash);
    }
    // save current tile
    yield this.put(xyKey, hash);
    var check = yield this.db.get(xyKey);
    console.log('motafuca', check);
