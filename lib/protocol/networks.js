/*!
 * network.js - bitcoin networks for decentraland
...
main.pow = {
  ...
  targetTimespan: 2 * 24 * 60 * 60, // two days
  ...
  targetSpacing: 6 * 60 * 60, // 6 hours, 4 per day
...
testnet.seeds = [
  'seed1.decentraland.org',
  'seed2.decentraland.org'
];

testnet.magic = 0x080a0c0f;
...
testnet.pow = {
  ...
  targetTimespan: 10 * 60, // ten minute retarget estimate
  targetSpacing: 1 * 60, // one minute blocks
  retargetInterval: 10,
  difficultyReset: true,
  noRetargeting: false
};

//console.log(btcutils.toCompact(testnet.pow.limit));
...
testnet.rpcPort = 8301;

testnet.contentPort = 9301;
...
const testnetGenesis = genesis({network: testnet, content: new Buffer('48c74f698189ec31f1d96c91793ffb944faa280e000000000000000000000000', 'hex')});
...
regtest.pow = {
  ...
  bits: 545259519,
  targetTimespan: 10 * 60, // 10 minutes
  targetSpacing: 1 * 60, // 1 minute
  retargetInterval: 10,
  difficultyReset: false,
  noRetargeting: false
};
...
const regtestGenesis = genesis({network: regtest});
regtest.genesisBlock = regtestGenesis.toRaw().toString('hex');
regtest.genesis.hash = regtestGenesis.hash().toString('hex');
regtest.genesis.merkleRoot = regtestGenesis.merkleRoot.toString();
