/*!
 * pool.js - peer management for decentraland
...
Pool.prototype._connect = co(function* connect() {
  ...
  if (this.hosts.size() === 0 && !this.options.bootstrapNetwork)
...
Pool.prototype.bindPeer = function bindPeer(peer) {
  ...
  peer.on('error', function(err) {
    if (!peer.isLoader()) {
      self.hosts.remove(peer.address.hostname)
    }
...
Pool.prototype.handleAddr = function handleAddr(addrs, peer) {
  ...
  for (i = 0; i < addrs.length; i++) {
    ...
    for (var name of ifnames) {
      var iface = ifaces[name];
      if ('IPv4' !== iface.family || iface.internal !== false) {
        continue;
      }
      if (iface.address === addr.host && iface.port === addr.port) {
        localMatches = true;
        break;
      }
    ...
    if ((addr.host === '127.0.0.1' || addr.host === this.address.host) &&
