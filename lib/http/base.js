/*!
 * http.js - http server for decentraland
...
HTTPBase.prototype.opt = function opt(path, callback, ctx) {
  if (Array.isArray(path)) {
    path.forEach(function(path) {
      this.opt(path, callback, ctx);
    }, this);
    return;
  }
  this.routes.options.push({ ctx: ctx, path: path, callback: callback });
};
