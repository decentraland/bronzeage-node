/*!
 * rpc.js - bitcoind-compatible json rpc for decentraland.
...
RPC.prototype.execute = function execute(json) {
  switch (json.method) {
    ...
    case 'startmining':
      return this.startmining(json.params);
    case 'getminerinfo':
      return this.getminerinfo(json.params);
    case 'stopmining':
      return this.stopmining(json.params);
    ...
    case 'fetchtile':
      return this.fetchtile(json.params);
    case 'gettile':
      return this.gettile(json.params);
    case 'gettileinfo':
      return this.gettileinfo(json.params);
    case 'settile':
      return this.settile(json.params);
    case 'isowntile':
      return this.isowntile(json.params);
    case 'transfertile':
      return this.transfertile(json.params);
    case 'dumpblockchain':
      return this.dumpblockchain(json.params);
...
RPC.prototype.gettile = co(function* gettile(args) {
  if (args.help || args.length !== 2)
    throw new RPCError('gettile "x", "y"');
  ...
  var tile = yield this.chain.getTile(x,y);
  return tile.content.toString('hex');
});

RPC.prototype.fetchtile = co(function* fetchtile(args) {
  if (args.help || args.length !== 2)
    throw new RPCError('fetchtile "x", "y"');

  var x = args[0];
  var y = args[1];

  var tile = this.chain.db.forceFetch(x, y);
});

RPC.prototype.gettileinfo = co(function* gettile(args) {
  if (args.help || args.length !== 2)
    throw new RPCError('gettile "x", "y"');
  ...
  var y = args[1];
  var tile = yield this.chain.getTile(x,y);
  const coin = yield this.chain.db.getTileAsCoin(x, y);
  return JSON.stringify(coin);
});

RPC.prototype.isowntile = co(function* isowntile(args) {
  if (args.help || args.length !== 2)
    throw new RPCError(`format: isowntile "x", "y"`);
  ...
  const coin = yield this.chain.db.getTileAsCoin(x, y);
  if (!coin) {
    throw new RPCError(`isowntile "${x}", "${y}" - no landbase in that position`);
  }

  const tile = yield this.chain.getTile(x, y);
  const script = tile.script;
  const hash = Address.fromScript(script).getHash();
  const key = yield this.wallet.getPrivateKey(hash);

  return key ? 'true' : 'false';
});


RPC.prototype.settile = co(function* settile(args) {
  if (args.help || args.length !== 3)
    throw new RPCError('settile "x", "y", "base64-encoded content"');
  ...
  const coin = yield this.chain.db.getTileAsCoin(x, y);
  if (!coin) {
    throw new RPCError('settile "x", "y" - no landbase in that position');
  }
  ...
  const key = yield this.wallet.getPrivateKey(hash);
  if (!key) {
    throw new RPCError('settile "x", "y" - no control of key for tile');
  }

  var content = yield this.chain.setTile(x, y, path);

  content = new Buffer(content+'000000000000000000000000', 'hex');

  try {
    const tx = yield this.wallet.createSimpleTX({
      x, y, content, coin, key
    });
    this.node.sendTX(tx).catch(e => {
      console.log('Could not broadcast tx', e);
    });
...
RPC.prototype.transfertile = co(function* transfertile(args) {
  if (args.help || args.length !== 3)
    throw new RPCError('transfertile "x", "y", "targetAddress"');
  ...
  try {
    const tx = yield this.wallet.createTransferTX({
      tile, coin, address, key
    });
    this.node.sendTX(tx).catch(e => {
      console.log('Could not broadcast tx', e);
    });
...
RPC.prototype.dumpblockchain = co(function* (args) {
  if (args.help || args.length > 1)
    throw new RPCError('dumpblockchain (only-controlled)')

  const onlyControlled = args.length === 1;

  const tiles = yield this.chain.getAllTiles();
  const result = [];
  const self = this;

  for (let i in tiles) {
    const { x, y, content } = tiles[i];
    const coin = yield this.chain.db.getTileAsCoin(x, y);
    const tile = yield this.chain.getTile(x, y);
    const script = tile.script;
    const address = Address.fromScript(script);
    const addrHash = address && address.getHash();
    const controlled = address && addrHash && !!(yield self.wallet.getPrivateKey(addrHash));

    if (!onlyControlled || onlyControlled && controlled) {
      result.push({
        x, y,
  ...
  return {
    ...
    vout: tx.outputs.map(function(output, i) {
      return {
        x: output.x,
        y: output.y,
        content: output.content.toString('hex'),
        n: i,
...
RPC.prototype.startmining = co(function* startmining(args) {
  if (this.miner.running || this.miner.stopping) {
    return
  }
  return yield this.miner.start()
});
RPC.prototype.getminerinfo = co(function* getminerinfo(args) {
  return yield {
    running: this.miner.running,
    stats: this.miner.lastStat
  };
});
RPC.prototype.stopmining = co(function* stopmining(args) {
  if (!this.miner.running || this.miner.stopping) {
    return
  }
  return yield this.miner.stop()
});
...
RPC.prototype.addmultisigaddress = co(function* addmultisigaddress(args) {
  ...
  // Impossible to implement in decentraland (no address book).
...
RPC.prototype.dumpwallet = co(function* dumpwallet(args) {
  ...
  time = util.date();
  out = [
    util.fmt('# Wallet Dump created by Decentraland %s', constants.USER_VERSION),
...
RPC.prototype._toListTX = co(function* _toListTX(wtx) {
  ...
  for (i = 0; i < details.outputs.length; i++) {
    ...
    if (member.path) {
      if (member.path.branch === 1 && !receive)
        continue;
      received += member.value || 0;
      recMember = member;
      recIndex = i;
      continue;
    }
...
RPC.prototype.listunspent = co(function* listunspent(args) {
  ...
  for (i = 0; i < coins.length; i++ ) {
    ...
    out.push({
      ...
      amount: Amount.btc(coin.value || 0, true),
      confirmations: depth,
...
RPC.prototype.setaccount = co(function* setaccount(args) {
  ...
  // Impossible to implement in decentraland:
  throw new Error('Not implemented.');
});
