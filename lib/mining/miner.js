/*!
 * miner.js - inefficient miner for decentraland (because we can)
 ...
 * @param {String?} [options.coinbaseFlags="mined by decentraland"]
...
function Miner(options) {
  ...
  this.version = -1;
  this.addresses = [];
  this.coinbaseFlags = new Buffer('mined by decentraland-node', 'ascii');
...
Miner.prototype._init = function _init() {
  ...
  this.chain.on('tip', function(tip) {
    ...
    if (self.attempt.block.prevBlock === tip.prevBlock || self.attempt.block.height < tip.height)
      self.attempt.destroy();
  });

  this.on('block', function(block, entry) {
    // Emit the block hex as a failsafe (in case we can't send it)
    const {x, y} = block.getCoinbaseCoords();
    self.logger.info('Found block: %d (%s) at (%d, %d).',
      entry.height, entry.rhash(), x, y);
    self.logger.debug('Raw: %s', block.toRaw().toString('hex'));
  });

  this.on('status', function(stat) {
    self.lastStat.hashrate = stat.hashrate / 1000 | 0;
    self.lastStat.hashes = stat.hashes;
    self.lastStat.target = stat.target;
    self.lastStat.height = stat.height;
    self.lastStat.best = stat.best;
    self.logger.info(
...
Miner.prototype.getBoundaryTile = co(function* () {
  return yield this.chain.getBoundaryTile();
});
...
Miner.prototype.createBlock = co(function* createBlock(tip, address) {
  ...
  const {x, y} = yield this.getBoundaryTile();
  assert(x !== undefined);
  assert(y !== undefined);
