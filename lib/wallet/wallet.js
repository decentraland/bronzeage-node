/*!
 * wallet.js - wallet object for decentraland
...
/**
 * Create a simple TX updating a tile
 */
Wallet.prototype.createSimpleTX = co(function* createTX(options, force) {

  assert(typeof options.x === 'number', 'Must supply x');
  assert(typeof options.y === 'number', 'Must supply y');
  assert(options.content, 'Must supply content');
  assert(options.coin, 'Must supply coins');
  assert(options.key, 'Must supply key to sign');
  ...
  tx.outputs.push(new Output({
      ...
      script: Script.fromAddress(this.getChange())
  }));

  tx.addInput(options.coin);
  const worked = tx.sign(options.key, 1) === 1;
  console.log('Correctly signed:', worked)

  return tx.toTX();
});

/**
 * Transfer a tile to target address
 */
Wallet.prototype.createTransferTX = co(function* createTX(options, force) {

  assert(options.tile, 'Must supply tile');
  assert(options.coin, 'Must supply coins');
  assert(options.address, 'Must supply target address');
  assert(options.key, 'Must supply key to sign');
  ...
  tx.outputs.push(new Output({
      x: options.tile.x,
      y: options.tile.y,
      content: options.tile.content,
      script: Script.fromAddress(options.address)
  }));
