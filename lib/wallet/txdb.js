...
TXDB.prototype.put = function put(key, value) {
  ...
  this.wallet.current.put(this.prefix(key), value || 1);
};
...
TXDB.prototype.resolveInput = co(function* resolveInput(tx, index, height, path) {
  ...
  if (stx.height === -1) {
    ...
    if (height !== -1)
      this.pending.confirmed += credit.coin.value || 1;
  }
...
TXDB.prototype.insert = co(function* insert(wtx, block) {
  ...
  if (!tx.isCoinbase()) {
    ...
    for (i = 0; i < tx.inputs.length; i++) {
      ...
      this.pending.coin--;
      this.pending.unconfirmed -= coin.value || 1;
      ...
      if (!block) {
        ...
        this.pending.confirmed -= coin.value || 1;
  ...
  for (i = 0; i < tx.outputs.length; i++) {
    ...
    this.pending.coin++;
    this.pending.unconfirmed += output.value || 1;

    if (block)
      this.pending.confirmed += output.value || 1;
  ...
  if (!tx.isCoinbase()) {
    ...
    for (i = 0; i < tx.inputs.length; i++) {
      ...
      if (!credit) {
        ...
        this.pending.coin--;
        this.pending.unconfirmed -= credit.coin.value || 1;
      }
...
TXDB.prototype.getWalletBalance = co(function* getWalletBalance() {
  ...
  for (i = 0; i < credits.length; i++) {
    ...
    if (coin.height !== -1)
      balance.confirmed += coin.value || 1;

    if (!credit.spent)
      balance.unconfirmed += coin.value || 1;
  }
